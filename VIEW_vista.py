...
def manejar_tecla(event):
    tecla = event.keysym
    if tecla == 'Return':  # La tecla Enter fue presionada
        MV_entradas.ingresar(tecla, entrada1)  # Llama a la función que se usa para procesar el '='
    elif tecla == 'BackSpace':  # La tecla Backspace fue presionada
        MV_entradas.borrar(entrada1)  # Llama a la función que se usa para borrar
    elif len(tecla) == 1:  # Si la tecla es un carácter imprimible (como un número o símbolo)
        # Permite que Tkinter maneje la inserción automáticamente
        return
    else:
        # Ignora cualquier otra tecla, como F1, Shift, Ctrl, etc.
        return
...
def seleccionar_historial():
    ventana_listbox = tk.Toplevel(root)
    ventana_listbox.title("Seleccionar Historial")
    ventana_listbox.geometry("350x400+250+150")
    ventana_listbox.resizable(False, False)

    frame_listbox = ttk.Frame(ventana_listbox)
    frame_listbox.pack(fill="both", expand=True, padx=10, pady=10)

    listbox = tk.Listbox(frame_listbox, selectmode="single", font=("Arial", 12), width=40, height=15)
    ...
    historial = firebase_python.RealtimeGet_Claves()
    
    item_claves = {}  
    
    for key, item in historial.items():  # Cambié de lista a diccionario
        listbox.insert(tk.END, item)
        item_claves[item] = key  # Guardar la relación valor -> key

    # Pasar el diccionario a la función RealtimeDelete_elemento
    listbox.bind("<<ListboxSelect>>", lambda event: firebase_python.RealtimeDelete_elemento(listbox, item_claves))
...
historial_menu.add_command(label="Seleccionar datos a borrar de base de datos", command=seleccionar_historial)
